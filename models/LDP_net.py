...
class extractor(nn.Module):
    def __init__(self, in_channel, out_channel=64):
        ...
        self.conv1 = nn.Conv2d(in_channel, out_channel // 4, kernel_size=3, stride=1, padding=1) #论文中的outchannel=128
        self.relu1 = nn.PReLU()
    ...
    def forward(self, x):
        ...
        x_2 = self.relu2(self.conv2(torch.cat([x_1, x], dim=1)))#第0维度是数据的batch_size 第一维度是光谱维度
        x_3 = self.relu3(self.conv3(torch.cat([x_2, x_1], dim=1)))

        out = self.relu4(self.conv4(torch.cat([x_3, x_2], dim=1)))
        #out代表输出的浅层特征 x3代表剩余链接的输出
        return out, x_3
