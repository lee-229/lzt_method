...
from function.loss import unsuper_loss,super_loss,unsuper_loss_LDP
...
def main(cfg, logger):
    ...
    if cfg.pretrained and cfg.test:
        ...
        if  (cfg.tradition==False and os.path.isfile(cfg.pretrained)) or cfg.tradition==True:
            if cfg.tradition==False:#深度学习方法才需要加载模型
            ...
            with open(cfg.csv_RR_dir) as csv_file:
                ...
                for r in row:
                    ...
                    PSNR.append(float(r[3]))  # 将字符串数据转化为浮点型加入到数组之中
                ...
                print('PSNR', round(np.mean(PSNR),4),round(np.var(PSNR),4))
        ...
        if cfg.resumeG:
            ...
            train(train_dataloader,valid_dataloader,valid_dataloader_2,G,G_resume,optim_G,G_loss_func,Smooth_operator,cfg,logger)

# training
def train(train_dataloader,valid_dataloader,valid_dataloader_2,
          G,G_resume,optim_G,G_loss_func,Smooth_operator,cfg ,logger):
    ...
    for epoch in range(cfg.start_epoch, cfg.num_epochs + 1):
        train_process(train_dataloader,valid_dataloader,valid_dataloader_2,
                      G,G_resume,optim_G, G_loss_func, Smooth_operator,epoch, cfg,logger)
        if epoch%10==0:
...
def test(test_dataloader, G, save_img_dir,test_type,cfg,logger):
    ...
    for idx, batch in enumerate(test_dataloader):
        ...
        if cfg.tradition==False:
            ...
            input_lr_up=torch2np(input_lr_up)
            prediction=[]
            if (cfg.ms_size == 16):
                for i in range(input_pan.shape[0]):
                    prediction.append(G(input_pan[i], input_lr[i]))
            else:
                for i in range(input_pan.shape[0]):
                    prediction.append(G(input_pan[i], input_lr_up[i])) 
        ...
        if test_type == 'test_low_res':
            header = ['image_index', 'SAM', 'ERGAS', 'MPSNR_numpy', 'SCC']
        else:
            header = ['image_index', 'D_lambda', 'D_s', 'QNR','SF','FCC']
            #返回字典
        results = eval_compute(input_pan, input_lr, prediction, target,test_type,cfg, logger)
        if test_type == 'test_low_res':
...
def train_process(dataloader,valid_dataloader,valid_dataloader_2,
    ...
    for iteration, batch in enumerate(dataloader):
        ...
        if cfg.train_type=='train_full_res':
            ...
            G_loss = G_loss_func(ms,ms_,input_lr,lr_ms,input_lr_up,gray_ms,
                pan_multi,lr_pan,lrms_up_gray)#无监督学习
        else:
            if(cfg.ms_size==16):
            ...
            #stage1 = G(input_pan, input_lr_up)
            G_loss = G_loss_func(input_lr,input_pan,stage1,target) #有监督学习
    ...
    with torch.no_grad():
        ...
        for idx, batch in enumerate(valid_dataloader_2):
            ...
            if cfg.train_type=='train_full_res':
                pan_multi = stack(input_pan, r=cfg.in_nc)
                prediction, _, _ ,_ , _ = G(input_lr_up, pan_multi)
                #prediction = prediction.to(cfg.device)
            ...
            for i in range(input_lr.shape[0]):
                #这里改成了sigmoid形式 删去 / 2 + 0.5
                ERGAS.append(ERGAS_numpy(target[i] , prediction[i] , sewar=False))
                SAM.append(SAM_numpy(target[i] , prediction[i] , sewar=False))
