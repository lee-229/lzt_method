...
def SAM_numpy(x_true, x_pred, sewar=False):
    ...
    dot_sum = np.sum(x_true * x_pred, axis=2)#H*W
    norm_true = norm(x_true, axis=2)#H*W
    norm_pred = norm(x_pred, axis=2)#H*W
    res = np.arccos(dot_sum / norm_pred / norm_true)
    is_nan = np.nonzero(np.isnan(res))#返回为nan的下标
...
def SAM_torch(x_true, x_pred):
    ...
    # res = torch.acos(res)
    # res = res / 3.14 #限制在0-1之间
    # res = torch.where(
    #     torch.isnan(res),
    #     torch.full_like(res, 0),
    #     res)
    sam = torch.mean(res)
    return sam
